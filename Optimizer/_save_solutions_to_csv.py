...
def save_solutions_to_csv(holder_of_solutions_and_info, **params):
    ...
    report_cdal_act = pd.DataFrame()
    report_cdal_pred = pd.DataFrame()
    ...
    cols_cdal_act = [f'CdAl{idx}-act' for idx in range(max_n_leaks)]
    cols_cdal_pred = [f'CdAl{idx}-pred' for idx in range(max_n_leaks)]
    ...
    for i in range(len(holder_of_solutions_and_info)):
        ...
        holder_cdal_act = holder_of_solutions_and_info[i][0][max_n_leaks:]
        holder_cdal_pred = []

        counter = 0
        for leak_size in holder_of_solutions_and_info[i][2]['CdAl']:
            holder_cdal_pred.append(leak_size)
        ...
        tmp_lst = []
        tmp_lst.append(holder_cdal_act)
        tmp_df = pd.DataFrame(tmp_lst, columns=cols_cdal_act)    
        report_cdal_act = pd.concat([report_cdal_act, tmp_df], axis=0, ignore_index=True)       

        tmp_lst = []
        tmp_lst.append(holder_cdal_pred)
        tmp_df = pd.DataFrame(tmp_lst, columns=cols_cdal_pred)    
        report_cdal_pred = pd.concat([report_cdal_pred, tmp_df], axis=0, ignore_index=True)  

    report = pd.concat([report_act, report_pred, report_xl, report_cdal_act, report_cdal_pred], axis=1)

    report.to_csv(report_directory + "/" + f'{model_name}-OnTest-{leak_pred}.csv', index=False)
